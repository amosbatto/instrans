...
/************************************************************************
File: instrans.php
License: GNU Lesser General Protection License (LGPL), version 3.0 or later
		which can be found at http://www.gnu.org 
Author: Amos B. Batto (amosbatto@yahoo.com)
Created: 22 Jun 2006 
Last Revised: 05 Mar 2021
 
instrans.php is a command line program which inserts translations from one PO 
file into another PO file. Tested with PHP 5.0 - 7.3.
...
include($sProgramPath . '/instrans-header.php'); 
include($sProgramPath . '/myfunc.php');
...
$bOrigDir = is_dir($sOrigPo);
$bTransDir = is_dir($sTransPo);

if (!$bOrigDir && !$bTransDir) //if both files
{
...
class PoObj
{
	...
	public function getStr()
	{
		...
		if (is_array($this->aPrevMsgid))
			foreach ($this->aPrevMsgid as $s)
				$sPo .= '#| ' . $s . "\n";
		
		if (is_string($this->sDomain))
			$sPo .= 'domain "' . $this->sDomain . "\"\n";
		
		if (is_array($this->aMsgctxt))
		{
			$sPo .= 'msgctxt ';
			foreach ($this->aMsgctxt as $s)
				$sPo .= '"' . $s . "\"\n";
		}	
	...
	//1: Comment, 2: domain, 3: msgctxt, 4: msgctxt continued line, 
	...
	public function addLine($sLine)
	{
		...
		elseif ($sLine[0] === '#')
		{
			...
			elseif ($sLine[1] === '|')      //if previous msgid (it has been changed)
			    $this->aPrevMsgid[] = ltrim(substr($sLine, 2));
		...
		{
			if ($iLastElement < SYNTAX_MSGCTXT) 
			{
			...
			elseif ($iLastElement == SYNTAX_MSGCTXT)
			{
				$this->aMsgctxt[] = substr($sLine, 1, -1);
		...
		elseif (preg_match('/^msgctxt[ \t]+"/', $sLine))
		{
			//if line doesn't terminate in double quotes, then syntax error
			if ($sLine[strlen($sLine) - 1] != '"') 
			{
				$sErr = 'msgctxt' . __(" no está encerrado en comillas.");
				$iRet = SYNTAX_ERR_NO_QUOTES;
				$sLine .= '"';
			}
			
			$this->aMsgctxt[] = substr(strstr($sLine, '"'), 1, -1);
								
			if ($iLastElement > SYNTAX_MSGCTXT)
			{
				$sErr =  'msgctxt' . __(" es definido afuera de la orden correcta de PO.");
				$iLastElement = SYNTAX_DOMAIN;
				return SYNTAX_ERR_ELEM_ORDER;
			}					
			else
			{
				$iLastElement = SYNTAX_MSGCTXT;
				return $iRet == SYNTAX_ERR_NO_QUOTES ? SYNTAX_ERR_NO_QUOTES : SYNTAX_MSGCTXT;
			}
		}
		elseif (preg_match('/^msgid[ \t]+"/', $sLine))	
		{
			//if line doesn't terminate in double quotes, then syntax error
			if ($sLine[strlen($sLine) - 1] != '"') 
			{
		...
		elseif (preg_match('/^msgstr[ \t]+"/', $sLine))
		{	
			//if line doesn't terminate in double quotes, then syntax error
			if ($sLine[strlen($sLine) - 1] != '"') 
			{
		...
		elseif (preg_match('/^msgid_plural[ \t]+"/', $sLine))
		{
			//if line doesn't terminate in double quotes, then syntax error
			if ($sLine[strlen($sLine) - 1] != '"') 
			{
		...
		elseif (preg_match('/^domain[ \t]+"/', $sLine))
		{
			//if line doesn't terminate in double quotes, then syntax error
			if ($sLine[strlen($sLine) - 1] != '"') 
			{
	...
	//returns 4 if msgid, reference, and automatic comments match or if the msgid and msgctxt match
	public function match(PoObj $po)
	{
		if ($this->strMsgid() === $po->strMsgid())
		{
			if ($this->strMsgctxt() === $po->strMsgctxt()) {
				return 4;
			}
	...
	public function clear()
	{
		$this->aTransCom = $this->aAutoCom = $this->aRef = $this->aFlag =
			$this->aPrevMsgid = $this->sDomain = $this->aMsgctxt =
			$this->aMsgid = $this->aMsgidPl = 
			$this->aMsgstr = $this->aMsgstrNo = null;
		return;
	}
	
	public function cntLines()
	{
		return myCount($this->aTransCom) + 
			myCount($this->aAutoCom) + myCount($this->aRef) + 
			myCount($this->aFlag) + myCount($this->aPrevMsgid) + 
			myCount($this->aMsgid) + myCount($this->aMsgstr) +
			myCount($this->sDomain) + myCount($this->aMsgctxt) +
			myCount($this->aMsgid) + myCount($this->aMsgidPl) +
			myCount($this->aMsgstr) + myCount($this->aMsgstrNo);
	}
	...
	public function strMsgctxt()
	{
		return is_array($this->aMsgctxt) ? implode('', $this->aMsgctxt) : null;
	}
...
function getArgs()
{
	global $argc, $argv, $sOrigPo, $sTransPo, $sNewPo, $bQuiet, $bShowPath,
		...
		$bBilingualIns, $sBilingualSeparator, $bMsgstrIns, 
		$bStatusBarIns, $sMenuSrch, $sStatusBarSrch, $bDebug, $bSearch,
		$bRecursive, $sCharSet, $sInterfaceLang, $aGetText, $sProgramPath, 
		$bDOS, $bStrip, $sStripStr, $bReplace, $sReplaceFind, $sReplaceWithWhat;
...
function findPoMatch(PoObj $oPo, $iSearchFirst = 0)
{
	...
	if (!is_int($iSearchFirst) || $iSearchFirst < 0 || myCount($aTransPo) <= $iSearchFirst)
		$iSearchFirst = 0;	
		...
		if ($retVal === 0 || feof($rOrigPoFile)) //if the end of the PO Object
		{
			...
			if ($posPo >= 0) //if match found
			{
				if ($bCommentIns)	//if inserting translation as a comment
				{
					...
					if ($bOverwrite)
					{
						if (is_array($curPo->aTransCom))
						{
							...
							{
								//only keep comments that do NOT match the form:
								// [FILENAME] "TRANSLATION" 
								// or 
								// [FILENAME] MENU: "TRANSLATION" (if bStatusBarIns) 
								if (0 === preg_match($bStatusBarIns ? '/^\[.+\] MENU: ".*"$/' : 
										'/^\[.+\] ".*"$/', $st)) 
									$aNew[] = $st;
							}
						}
				...
				if($bBilingualIns) 
				{
					...
					{
						$cntArray = myCount($curPo->aMsgstr);
				...
				elseif($bMsgstrIns) 	//if inserting translation as the msgstr
				{
					if ($bOverwrite || $curPo->aMsgstr === null || 
						($curPo->aMsgstr[0] === '' && myCount($curPo->aMsgstr) == 1))
					{	
						$curPo->aMsgstr = null;	
...
function srchIndexTransPo($sFind)
{
	...
	$begin = 0;
	$end = myCount($aIndexTransPo) - 1; //there is a least one element in array, so safe to subtract 1
...
function perlStr($str)
{
	...
	elseif (strlen($str) > 0 && preg_match('/^[[:space:]]*/.*/[igmsxUX]*[[:space:]]*$/', $str))
		return $str;
